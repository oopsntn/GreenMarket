...
function AnalyticsPage() {
  ...
  const maxImpressions = Math.max(
    ...placementChartRows.map((item) => item.impressions),
    1,
  );
            ...
            <div className="analytics-chart-placeholder analytics-chart-placeholder--bar">
              <div className="analytics-chart-caption">
                ...
                <span className="analytics-chart-caption__text">
                  Each bar shows total views generated by a placement slot in the selected period.
                </span>
              </div>

              <div className="analytics-bars">
                {placementChartRows.map((item, index) => (
                  <div key={item.slot} className="analytics-bar-group">
                    <span className="analytics-bar-group__value">
                      {formatCompactMetric(item.impressions)}
                    </span>
                    <div className="analytics-bar-group__track">
                      <div
                        className="analytics-bar-group__bar"
                        style={{
                          height: `${Math.max(
                            18,
                            (item.impressions / maxImpressions) * 100,
                          )}%`,
                          backgroundColor: chartPalette[index % chartPalette.length],
                        }}
                      />
                    </div>
                    <span className="analytics-bar-group__label">{item.slot}</span>
                  </div>
                ))}
              </div>
            </div>
