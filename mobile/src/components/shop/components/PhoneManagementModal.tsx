...
import { X, Plus, Trash2, ShieldCheck, Star, Send, Crown } from 'lucide-react-native';
...
interface PhoneManagementModalProps {
    visible: boolean;
    onClose: () => void;
    /** Toàn bộ danh sách SĐT: phones[0] = SĐT chính (đăng nhập OTP) */
    phones: string[];
    /** Email shop đã xác thực — cần để xác thực khi đổi SĐT chính */
    shopEmail?: string;
...
const PhoneManagementModal: React.FC<PhoneManagementModalProps> = ({
    ...
    const handleInitSetPrimary = async (phone: string) => {
        ...
        // Gửi OTP về email để xác thực danh tính
        setLoading(true);
        try {
            await ShopService.requestVerifyOTP(shopEmail, 'email');
            setTargetPrimary(phone);
            setStep('SET_PRIMARY_OTP');
            CustomAlert('Đã gửi', `Mã xác thực đã được gửi đến email ${shopEmail}. Nhập mã để xác nhận đổi SĐT chính.`);
        } catch (error: any) {
    ...
    const handleConfirmSetPrimary = async () => {
        ...
        try {
            ...
            // Reorder local phones array theo kết quả backend
            ...
            CustomAlert(
                'Thành công',
                `Số ${targetPrimary} đã trở thành SĐT đăng nhập chính. Token hiện tại vẫn còn hiệu lực cho đến khi bạn đăng xuất và đăng nhập lại.`
            );
            resetState();
        } catch (error: any) {
            ...
            <View style={styles.overlay}>
                <View style={styles.modalContent}>
                    ...
                    <Text style={styles.subtitle}>
                        Tối đa 3 số điện thoại. Số <Text style={{ fontWeight: '800', color: '#1e293b' }}>Chính</Text> dùng để đăng nhập OTP, 2 số còn lại là dự phòng. Có thể đổi số dự phòng đã xác thực làm số chính (cần xác thực qua email cửa hàng).
                    </Text>
                                ...
                                return (
                                    <View key={p} style={[styles.phoneItem, isPrimary && styles.primaryPhoneItem]}>
                                        ...
                                        <View style={styles.phoneActions}>
                                            {/* Nút đặt làm số chính (chỉ cho số phụ) */}
                                            {!isPrimary && step === 'IDLE' && (
                                                <TouchableOpacity
                    ...
                    {step === 'ADD_OTP' && (
                        <View style={styles.addSection}>
                            <Text style={styles.infoText}>
                                Nhập mã OTP đã gửi đến <Text style={{ fontWeight: '800' }}>{newPhone}</Text>
                            </Text>
                            <Input label="Mã OTP xác thực SĐT" placeholder="6 chữ số" value={addOtp} onChangeText={setAddOtp} type="number-pad" />
                            <View style={styles.actionRow}>
                                <Button variant="outline" style={{ flex: 1, marginRight: 8 }} onPress={resetState} disabled={loading}>
                                    Hủy
                                </Button>
                                <Button style={{ flex: 1 }} onPress={handleConfirmAddPhone} loading={loading} disabled={loading || !addOtp.trim()}>
                                    Xác nhận
                                </Button>
                            </View>
                        </View>
                        ...
                        <View style={styles.addSection}>
                            <View style={styles.warningBox}>
                                <Text style={styles.warningText}>
                                    ⚠️ Bạn đang đổi SĐT đăng nhập chính thành{' '}
                                    <Text style={{ fontWeight: '800' }}>{targetPrimary}</Text>.{'\n'}
                                    Nhập mã OTP đã gửi đến email{' '}
                                    <Text style={{ fontWeight: '800' }}>{shopEmail}</Text> để xác nhận.
                                </Text>
                            </View>
                            <Input label="Mã OTP từ Email" placeholder="6 chữ số" value={emailOtp} onChangeText={setEmailOtp} type="number-pad" icon={<Send size={16} color="#94a3b8" />} />
                            <View style={styles.actionRow}>
                                <Button variant="outline" style={{ flex: 1, marginRight: 8 }} onPress={resetState} disabled={loading}>
                                    Hủy
                                </Button>
                                <Button
                                    style={{ flex: 2, backgroundColor: '#d97706' }}
...
const styles = StyleSheet.create({
    ...
    emptyText: { color: '#94a3b8', fontStyle: 'italic', textAlign: 'center', paddingVertical: 10 },
    phoneItem: {
        flexDirection: 'row',
        justifyContent: 'space-between',
        alignItems: 'center',
    ...
    phoneInfo: { flexDirection: 'row', alignItems: 'center', flex: 1 },
    phoneText: { fontSize: 15, fontWeight: '600', color: '#334155' },
    primaryPhoneText: { color: '#92400e' },
    phoneBadge: { fontSize: 10, fontWeight: '600', marginTop: 2 },
    primaryBadge: { color: '#d97706' },
    secondaryBadge: { color: '#64748b' },
    phoneActions: { flexDirection: 'row', alignItems: 'center', gap: 8 },
    setPrimaryBtn: {
        padding: 6,
    ...
    infoText: { fontSize: 13, color: '#3b82f6', marginBottom: 10, fontWeight: '500' },
