...
import { Check, X, MailOpen, Building2 } from 'lucide-react-native';
...
const InvitationsScreen = () => {
    ...
    const handleAction = async (id: number, action: 'accept' | 'reject') => {
        ...
        try {
            ...
            CustomAlert(
                'Thành công', 
                action === 'accept' ? 'Bạn đã chấp nhận lời mời, bây giờ bạn có thể thao tác với shop này.' : 'Đã từ chối lời mời'
            );
            fetchInvitations();
        } catch (error: any) {
    ...
    const confirmReject = (id: number) => {
        CustomAlert(
            'Từ chối lời mời',
            'Bạn có chắc chắn muốn bỏ qua lời mời từ shop này không?',
            [
            ...
            <View style={styles.card}>
                <View style={styles.cardInfo}>
                    <View style={styles.imgContainer}>
                        {item.shopLogoUrl ? (
                            <Image 
                                source={{ uri: resolveImageUrl(item.shopLogoUrl) }} 
                    ...
                    <View style={styles.textContainer}>
                        ...
                        <Text style={styles.ownerName}>
                            Chủ vườn: {item.shopOwnerName || 'Unknown'}
                        </Text>
