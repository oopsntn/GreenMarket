...
import ModeratorService, { ShopModerationData } from '../services/ModeratorService';
import CustomAlert from '../../utils/AlertHelper';

const ShopModerationList = ({ navigation }: any) => {
  ...
  const fetchShops = async () => {
    try {
      setLoading(true);
      const data = await ModeratorService.getShops();
      // Show only pending or all? Let's show those with 'Pending' status as defined in controller
  ...
  const handleVerify = (id: number) => {
    CustomAlert('Verify Shop', 'Confirm this shop is eligible to operate on the platform?', [
      ...
      { 
        ...
        onPress: async () => {
          try {
            await ModeratorService.verifyShop(id);
  ...
  const renderItem = ({ item }: { item: ShopModerationData }) => (
    <TouchableOpacity 
      style={styles.card}
      onPress={() => navigation.navigate('ShopModerationDetail', { shopId: item.id })}
      activeOpacity={0.7}
    >
...
export default ShopModerationList;
