    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-background">
      {shops.length === 0 ? (
        <div className="text-center py-24 bg-white border border-slate-200 rounded-4xl shadow-sm border-dashed">
          ...
          <h3 className="text-xl font-black mb-2 text-slate-900 tracking-tight uppercase">
            Chưa có nhà vườn nào
          </h3>
          <p className="text-slate-500 max-w-xs mx-auto font-medium">
            Hiện tại chưa có nhà vườn nào hoạt động trên hệ thống.
          </p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {shops.map((shop) => {
              const isVip = Boolean(shop.shopIsVipActive);

              return (
                <Link
                  key={shop.shopId}
                  to={`/shop/${shop.shopId}`}
                  className={`group bg-white p-8 rounded-4xl border transition-all duration-300 block shadow-sm hover:shadow-2xl relative overflow-hidden ${isVip
                    ? 'border-amber-300 hover:border-amber-500/60 bg-gradient-to-br from-amber-50/60 via-white to-amber-100/20'
                    : 'border-slate-200 hover:border-emerald-500/30'
                    }`}
                >
                  <div
                    className={`absolute top-0 right-0 w-32 h-32 blur-3xl -mr-16 -mt-16 ${isVip ? 'bg-amber-500/15' : 'bg-emerald-500/5'
                      }`}
                  ></div>

                  <div className="flex items-center gap-6 mb-6">
                    <div
                      className={`w-20 h-20 rounded-3xl bg-slate-50 flex items-center justify-center overflow-hidden shadow-sm group-hover:scale-110 transition-transform ${isVip ? 'border border-amber-300' : 'border border-slate-100'
                        }`}
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      <h2
                        className={`text-xl font-black line-clamp-1 transition-colors uppercase tracking-tight leading-tight mb-2 ${isVip ? 'text-amber-900 group-hover:text-amber-700' : 'text-slate-900 group-hover:text-emerald-700'
                          }`}
                      >
                        {shop.shopName}
                      </h2>
                      <div className="flex flex-wrap items-center gap-2">
                        {isVip ? (
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-100 text-amber-800 text-[10px] uppercase font-black tracking-widest border border-amber-300 shadow-sm">
                            <Crown className="w-3.5 h-3.5" /> VIP
                          </span>
                        ) : null}
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-[10px] uppercase font-black tracking-widest border border-emerald-100 shadow-sm">
