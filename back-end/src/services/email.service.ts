...
export class EmailService {
    ...
    constructor() {
        ...
        if (!user || !pass) {
            console.warn("[EMAIL SERVICE] Warning: SMTP credentials missing (SMTP_USER or SMTP_PASS). Email sending will fail.");
        }
        ...
        this.transporter = nodemailer.createTransport({
            ...
            // Add some timeout for reliability
            connectionTimeout: 10000, // 10 seconds
            greetingTimeout: 10000,
            socketTimeout: 20000,
        });

        // Test the connection on startup (Silent)
        this.transporter.verify((error) => {
            if (error) {
                console.error("[EMAIL SERVICE] Connection validation failed:", error.message);
                if (error.message.includes("socket close")) {
                    console.error("[EMAIL SERVICE] Tip: Check if your IP is blocked or firewall settings. Try port 587 if 465 fails.");
                }
            } else {
                console.log("[EMAIL SERVICE] SMTP Server is ready to deliver messages.");
            }
        });
    }
    ...
    async sendOTPEmail(to: string, code: string): Promise<{ success: boolean; message: string }> {
        const html = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; padding: 20px;">
                ...
                <div style="background-color: #f9f9f9; border-radius: 8px; padding: 20px; text-align: center;">
                    ...
                    <p style="color: #555; font-size: 16px;">Vui lòng sử dụng mã dưới đây để hoàn tất quy trình xác minh email trên GreenMarket.</p>
        ...
        return this.sendMail(to, "Mã xác thực Email - GreenMarket", html);
    }
    ...
    async sendSecurityWarningEmail(to: string, phone: string): Promise<{ success: boolean; message: string }> {
        const html = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #f44336; border-radius: 8px; padding: 20px;">
                ...
                <p>Nếu bạn **không** thực hiện thay đổi này, tài khoản của bạn có thể đang bị xâm nhập. Vui lòng kiểm tra lại ngay lập tức.</p>
                <div style="margin-top: 20px; color: #777; font-size: 12px; text-align: center;">
                    <p>&copy; 2026 GreenMarket Team. All rights reserved.</p>
                </div>
            </div>
        ...
        return this.sendMail(to, "Cảnh báo bảo mật: Thay đổi thông tin - GreenMarket", html);
    }
